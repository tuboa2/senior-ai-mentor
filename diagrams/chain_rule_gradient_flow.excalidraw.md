---

excalidraw-plugin: parsed
tags: [excalidraw, senior-ai-mentor]

---
==⚠  Switch to EXCALIDRAW VIEW in the MORE OPTIONS menu of this document. ⚠== You can decompress Drawing data with the command palette: 'Decompress current Excalidraw file'. For more info check in plugin settings under 'Saving'

# Computational Graph & Backpropagation

## 🌿 1. Concept Intuition & Mental Model
Backpropagation is reverse-mode automatic differentiation applied to an acyclic computation graph:
- **Forward:** Information cascades from input variables to compute objective scalar loss $\mathcal{L}$.
- **Backward:** A scalar perturbation of 1.0 at $\mathcal{L}$ propagates upstream, calculating sensitivity derivatives $\frac{\partial \mathcal{L}}{\partial \theta}$ for every parameter.

---

## 📐 2. Mathematical Foundations (KaTeX / LaTeX)
### Forward Equations

$$\mathbf{z}^{[l]} = \mathbf{W}^{[l]} \mathbf{a}^{[l-1]} + \mathbf{b}^{[l]}, \quad \mathbf{a}^{[l]} = \sigma(\mathbf{z}^{[l]})$$

### Backward Adjoint Error Vector

$$\boldsymbol{\delta}^{[l]} \equiv \frac{\partial \mathcal{L}}{\partial \mathbf{z}^{[l]}} = \left( (\mathbf{W}^{[l+1]})^T \boldsymbol{\delta}^{[l+1]} \right) \odot \sigma'(\mathbf{z}^{[l]})$$

### Parameter Gradient Outer Products

$$\frac{\partial \mathcal{L}}{\partial \mathbf{W}^{[l]}} = \boldsymbol{\delta}^{[l]} (\mathbf{a}^{[l-1]})^T, \quad \frac{\partial \mathcal{L}}{\partial \mathbf{b}^{[l]}} = \sum_{\text{batch}} \boldsymbol{\delta}^{[l]}$$

---

## 💡 3. Senior Engineering & Performance Notes
- **Gradient Saturation Trap:** If sigmoid or tanh activations are used, saturation occurs when $|z| \gg 1$, driving $\sigma'(z) \to 0$ and killing gradient flow. ReLU, GELU, and SwiGLU avoid negative saturation.
- **Gradient Checkpointing:** Recomputes activations dynamically in the backward pass rather than caching all layer outputs, reducing peak activation VRAM by $4\times$.

---

# Excalidraw Data

## Text Elements
Libraries ^pCM4BpSl

torch.autograd ^YGpIcA8v

torch.nn ^UFqCo45Y

Methods ^XHcFcMe7

Architecture ^ifZj6Go9

Computational Graph & Backpropagation ^HRwSmNWc

Forward Activation Pass ^rym2lQ1s

z = torch.matmul(W, a_prev) + b ^BL0bJ6g2

Computes affine linear weighted sum of upstream layer activations. ^rN7gnC66

a = torch.relu(z) ^ESshHN1v

Applies non-linear activation gate producing layer representations. ^9e5QcRR8

Activation Caching Requirement: Intermediate tensors a_prev and z must be retained in memory during forward pass to compute gradients during backward pass. ^39Fa4R1U

Reverse Adjoint Chain Rule ^Dcpg7PSO

delta = W_next.T @ delta_next * relu'(z) ^f47TfdFX

Propagates adjoint error vector delta upstream via transposed weights and local derivative. ^PWFDf8MN

grad_W = torch.outer(delta, a_prev) ^2dBPgAOJ

Calculates exact weight gradient as outer product of error and activations. ^PCn8ab18

Reverse-Mode Auto-Differentiation: Chain Rule: dL/dW = delta * (a_prev)^T Adjoint error vectors propagate perturbation backward in reverse topological order in O(1) pass. ^Zfro7qRb

Optimizer Update Step ^NK8YDWe4

W.data.add_(-lr * grad_W) ^t3MEEid7

Adjusts weights against the direction of steepest loss ascent. ^XqWbeJ0p

optimizer.zero_grad( set_to_none=True) ^PdGVtqy7

Frees accumulated gradient buffers to prevent gradient leakage across training steps. ^7IRToi2t

Numerical Stability in Deep Nets: Gradient clipping bounds parameter updates. AMP dynamic loss scaling avoids FP16 underflow to zero on subtle weight adjustments. ^tQRVM68K

Input Tensor a_0 ^NpwTGR5R

Affine Linear Sum Wx+b ^LyMIXPw4

Activation Gate sigma(z) ^0gVbNCxy

Loss Criterion L(y, y_hat) ^wBMkhNWy

Accumulate Gradients ^Wl7YjRwj

Compute Loss Adjoint dL/dy_hat ^3yNoZKWG

%%
## Drawing
```json
{
  "type": "excalidraw",
  "version": 2,
  "source": "https://excalidraw.com",
  "elements": [
    {
      "id": "O0KXUly2qrpOQP17",
      "type": "rectangle",
      "x": 560.0,
      "y": 37.0,
      "width": 181.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 145465,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "pCM4BpSl"
        },
        {
          "type": "arrow",
          "id": "cGBq9TQA"
        },
        {
          "type": "arrow",
          "id": "JjsELDxq"
        },
        {
          "type": "arrow",
          "id": "PJpNKfBh"
        }
      ],
      "updated": 1789190079204,
      "link": null,
      "locked": false
    },
    {
      "id": "pCM4BpSl",
      "type": "text",
      "x": 610.0,
      "y": 57.5,
      "width": 81.0,
      "height": 23.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 1384380,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079204,
      "link": null,
      "locked": false,
      "text": "Libraries",
      "rawText": "Libraries",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "O0KXUly2qrpOQP17",
      "originalText": "Libraries",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "9fik7KGMSoGytFTY",
      "type": "rectangle",
      "x": 880.0,
      "y": -5.5,
      "width": 181.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 5689005,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "YGpIcA8v"
        },
        {
          "type": "arrow",
          "id": "cGBq9TQA"
        }
      ],
      "updated": 1789190079204,
      "link": null,
      "locked": false
    },
    {
      "id": "YGpIcA8v",
      "type": "text",
      "x": 907.5,
      "y": 15.0,
      "width": 126.0,
      "height": 23.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2798434,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079204,
      "link": null,
      "locked": false,
      "text": "torch.autograd",
      "rawText": "torch.autograd",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "9fik7KGMSoGytFTY",
      "originalText": "torch.autograd",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "cGBq9TQA",
      "type": "arrow",
      "x": 741.0,
      "y": 69.0,
      "width": 139.0,
      "height": 42.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 1673977,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          139.0,
          -42.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "O0KXUly2qrpOQP17",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "9fik7KGMSoGytFTY",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "YOkHRz4zR8LvISl1",
      "type": "rectangle",
      "x": 880.0,
      "y": 79.5,
      "width": 181.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 5042941,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "UFqCo45Y"
        },
        {
          "type": "arrow",
          "id": "JjsELDxq"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "UFqCo45Y",
      "type": "text",
      "x": 934.5,
      "y": 100.0,
      "width": 72.0,
      "height": 23.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 1606043,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "torch.nn",
      "rawText": "torch.nn",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "YOkHRz4zR8LvISl1",
      "originalText": "torch.nn",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "JjsELDxq",
      "type": "arrow",
      "x": 741.0,
      "y": 69.0,
      "width": 139.0,
      "height": 42.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 749603,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          139.0,
          42.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "O0KXUly2qrpOQP17",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "YOkHRz4zR8LvISl1",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "gbOfEVJIG5WdjTLU",
      "type": "rectangle",
      "x": 560.0,
      "y": 511.875,
      "width": 181.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7531730,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "XHcFcMe7"
        },
        {
          "type": "arrow",
          "id": "dpQi9rSQ"
        },
        {
          "type": "arrow",
          "id": "N73ylexH"
        },
        {
          "type": "arrow",
          "id": "6a9vQddJ"
        },
        {
          "type": "arrow",
          "id": "uhB2hBsH"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "XHcFcMe7",
      "type": "text",
      "x": 619.0,
      "y": 532.4,
      "width": 63.0,
      "height": 23.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6001212,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "Methods",
      "rawText": "Methods",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "gbOfEVJIG5WdjTLU",
      "originalText": "Methods",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "ctRogZEC6MxttOQf",
      "type": "rectangle",
      "x": 560.0,
      "y": 1150.0,
      "width": 181.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 3082730,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "ifZj6Go9"
        },
        {
          "type": "arrow",
          "id": "SEGOVK0G"
        },
        {
          "type": "arrow",
          "id": "md8eltjB"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "ifZj6Go9",
      "type": "text",
      "x": 596.5,
      "y": 1170.5,
      "width": 108.0,
      "height": 23.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 4476181,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "Architecture",
      "rawText": "Architecture",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "ctRogZEC6MxttOQf",
      "originalText": "Architecture",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "c0ULSyG9DSRpN0Az",
      "type": "rectangle",
      "x": 140.0,
      "y": 588.0,
      "width": 220.0,
      "height": 75.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7082509,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "HRwSmNWc"
        },
        {
          "type": "arrow",
          "id": "PJpNKfBh"
        },
        {
          "type": "arrow",
          "id": "dpQi9rSQ"
        },
        {
          "type": "arrow",
          "id": "SEGOVK0G"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "HRwSmNWc",
      "type": "text",
      "x": 164.5,
      "y": 602.5,
      "width": 171.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7610116,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "Computational Graph\n& Backpropagation",
      "rawText": "Computational Graph & Backpropagation",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "c0ULSyG9DSRpN0Az",
      "originalText": "Computational Graph\n& Backpropagation",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "PJpNKfBh",
      "type": "arrow",
      "x": 360.0,
      "y": 625.5,
      "width": 200.0,
      "height": 556.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 3520018,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          200.0,
          -556.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "c0ULSyG9DSRpN0Az",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "O0KXUly2qrpOQP17",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "dpQi9rSQ",
      "type": "arrow",
      "x": 360.0,
      "y": 625.5,
      "width": 200.0,
      "height": 81.625,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 192859,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          200.0,
          -81.625
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "c0ULSyG9DSRpN0Az",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "gbOfEVJIG5WdjTLU",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "SEGOVK0G",
      "type": "arrow",
      "x": 360.0,
      "y": 625.5,
      "width": 200.0,
      "height": 556.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 8704796,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          200.0,
          556.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "c0ULSyG9DSRpN0Az",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "ctRogZEC6MxttOQf",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "mokPwv2DqhUAaSnG",
      "type": "rectangle",
      "x": 880.0,
      "y": 267.0,
      "width": 200.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 9327463,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "rym2lQ1s"
        },
        {
          "type": "arrow",
          "id": "N73ylexH"
        },
        {
          "type": "arrow",
          "id": "PDVYYVS4"
        },
        {
          "type": "arrow",
          "id": "E650vecf"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "rym2lQ1s",
      "type": "text",
      "x": 899.0,
      "y": 276.0,
      "width": 162.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2740455,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "Forward Activation\nPass",
      "rawText": "Forward Activation Pass",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "mokPwv2DqhUAaSnG",
      "originalText": "Forward Activation\nPass",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "N73ylexH",
      "type": "arrow",
      "x": 741.0,
      "y": 543.875,
      "width": 139.0,
      "height": 244.875,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 4913466,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          139.0,
          -244.875
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "gbOfEVJIG5WdjTLU",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "mokPwv2DqhUAaSnG",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "Wj2zt1VIc57MOnGx",
      "type": "rectangle",
      "x": 1200.0,
      "y": 220.0,
      "width": 240.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 8314930,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "BL0bJ6g2"
        },
        {
          "type": "arrow",
          "id": "PDVYYVS4"
        },
        {
          "type": "arrow",
          "id": "miI7hfRl"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "BL0bJ6g2",
      "type": "text",
      "x": 1234.5,
      "y": 229.0,
      "width": 171.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 5008559,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "z = torch.matmul(W,\na_prev) + b",
      "rawText": "z = torch.matmul(W, a_prev) + b",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "Wj2zt1VIc57MOnGx",
      "originalText": "z = torch.matmul(W,\na_prev) + b",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "PDVYYVS4",
      "type": "arrow",
      "x": 1080.0,
      "y": 299.0,
      "width": 120.0,
      "height": 47.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 1988393,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          -47.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "mokPwv2DqhUAaSnG",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "Wj2zt1VIc57MOnGx",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "yfnw7Y6Sxl7053BM",
      "type": "rectangle",
      "x": 1560.0,
      "y": 220.0,
      "width": 360.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 190035,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "rN7gnC66"
        },
        {
          "type": "arrow",
          "id": "miI7hfRl"
        },
        {
          "type": "arrow",
          "id": "2mhz8ZM9"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "rN7gnC66",
      "type": "text",
      "x": 1587.0,
      "y": 229.0,
      "width": 306.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 8916182,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "Computes affine linear weighted\nsum of upstream layer activations.",
      "rawText": "Computes affine linear weighted sum of upstream layer activations.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "yfnw7Y6Sxl7053BM",
      "originalText": "Computes affine linear weighted\nsum of upstream layer activations.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "miI7hfRl",
      "type": "arrow",
      "x": 1440.0,
      "y": 252.0,
      "width": 120.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 7161353,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "Wj2zt1VIc57MOnGx",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "yfnw7Y6Sxl7053BM",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "VGcreu2b6VdyNsdl",
      "type": "rectangle",
      "x": 1200.0,
      "y": 314.0,
      "width": 240.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 9539883,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "ESshHN1v"
        },
        {
          "type": "arrow",
          "id": "E650vecf"
        },
        {
          "type": "arrow",
          "id": "8EBU59Go"
        }
      ],
      "updated": 1789190079205,
      "link": null,
      "locked": false
    },
    {
      "id": "ESshHN1v",
      "type": "text",
      "x": 1243.5,
      "y": 334.5,
      "width": 153.0,
      "height": 23.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2489059,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079205,
      "link": null,
      "locked": false,
      "text": "a = torch.relu(z)",
      "rawText": "a = torch.relu(z)",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "VGcreu2b6VdyNsdl",
      "originalText": "a = torch.relu(z)",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "E650vecf",
      "type": "arrow",
      "x": 1080.0,
      "y": 299.0,
      "width": 120.0,
      "height": 47.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 7267028,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079206,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          47.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "mokPwv2DqhUAaSnG",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "VGcreu2b6VdyNsdl",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "1XyDlUe3OLsabqcp",
      "type": "rectangle",
      "x": 1560.0,
      "y": 314.0,
      "width": 360.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 3779806,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "9e5QcRR8"
        },
        {
          "type": "arrow",
          "id": "8EBU59Go"
        },
        {
          "type": "arrow",
          "id": "vIBdu0MC"
        }
      ],
      "updated": 1789190079206,
      "link": null,
      "locked": false
    },
    {
      "id": "9e5QcRR8",
      "type": "text",
      "x": 1587.0,
      "y": 323.0,
      "width": 306.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 1796078,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079206,
      "link": null,
      "locked": false,
      "text": "Applies non-linear activation gate\nproducing layer representations.",
      "rawText": "Applies non-linear activation gate producing layer representations.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "1XyDlUe3OLsabqcp",
      "originalText": "Applies non-linear activation gate\nproducing layer representations.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "8EBU59Go",
      "type": "arrow",
      "x": 1440.0,
      "y": 346.0,
      "width": 120.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 5875810,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079206,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "VGcreu2b6VdyNsdl",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "1XyDlUe3OLsabqcp",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "BIeGRptm54RiCvJu",
      "type": "rectangle",
      "x": 2060.0,
      "y": 236.5,
      "width": 380.0,
      "height": 125.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 1776724,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "39Fa4R1U"
        },
        {
          "type": "arrow",
          "id": "2mhz8ZM9"
        },
        {
          "type": "arrow",
          "id": "vIBdu0MC"
        }
      ],
      "updated": 1789190079206,
      "link": null,
      "locked": false
    },
    {
      "id": "39Fa4R1U",
      "type": "text",
      "x": 2101.5,
      "y": 241.5,
      "width": 297.0,
      "height": 115.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2551606,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079206,
      "link": null,
      "locked": false,
      "text": "Activation Caching Requirement:\nIntermediate tensors a_prev and z\nmust be retained in memory during\nforward pass to compute gradients\nduring backward pass.",
      "rawText": "Activation Caching Requirement: Intermediate tensors a_prev and z must be retained in memory during forward pass to compute gradients during backward pass.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "BIeGRptm54RiCvJu",
      "originalText": "Activation Caching Requirement:\nIntermediate tensors a_prev and z\nmust be retained in memory during\nforward pass to compute gradients\nduring backward pass.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "2mhz8ZM9",
      "type": "arrow",
      "x": 1920.0,
      "y": 252.0,
      "width": 140.0,
      "height": 47.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 5387169,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079206,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          140.0,
          47.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "yfnw7Y6Sxl7053BM",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "BIeGRptm54RiCvJu",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "vIBdu0MC",
      "type": "arrow",
      "x": 1920.0,
      "y": 346.0,
      "width": 140.0,
      "height": 47.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 8704641,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079206,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          140.0,
          -47.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "1XyDlUe3OLsabqcp",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "BIeGRptm54RiCvJu",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "mmBRp72ZBjsEwIKe",
      "type": "rectangle",
      "x": 880.0,
      "y": 510.0,
      "width": 200.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7260123,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "Dcpg7PSO"
        },
        {
          "type": "arrow",
          "id": "6a9vQddJ"
        },
        {
          "type": "arrow",
          "id": "G3hdtztp"
        },
        {
          "type": "arrow",
          "id": "PS6eyebr"
        }
      ],
      "updated": 1789190079209,
      "link": null,
      "locked": false
    },
    {
      "id": "Dcpg7PSO",
      "type": "text",
      "x": 912.5,
      "y": 519.0,
      "width": 135.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 3592211,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "text": "Reverse Adjoint\nChain Rule",
      "rawText": "Reverse Adjoint Chain Rule",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "mmBRp72ZBjsEwIKe",
      "originalText": "Reverse Adjoint\nChain Rule",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "6a9vQddJ",
      "type": "arrow",
      "x": 741.0,
      "y": 543.875,
      "width": 139.0,
      "height": 1.875,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 9919601,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          139.0,
          -1.875
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "gbOfEVJIG5WdjTLU",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "mmBRp72ZBjsEwIKe",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "crP78EQCqvjbbS4V",
      "type": "rectangle",
      "x": 1200.0,
      "y": 455.5,
      "width": 240.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 832705,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "f47TfdFX"
        },
        {
          "type": "arrow",
          "id": "G3hdtztp"
        },
        {
          "type": "arrow",
          "id": "0hOSezZ8"
        }
      ],
      "updated": 1789190079209,
      "link": null,
      "locked": false
    },
    {
      "id": "f47TfdFX",
      "type": "text",
      "x": 1225.5,
      "y": 464.5,
      "width": 189.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 1515486,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "text": "delta = W_next.T @\ndelta_next * relu'(z)",
      "rawText": "delta = W_next.T @ delta_next * relu'(z)",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "crP78EQCqvjbbS4V",
      "originalText": "delta = W_next.T @\ndelta_next * relu'(z)",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "G3hdtztp",
      "type": "arrow",
      "x": 1080.0,
      "y": 542.0,
      "width": 120.0,
      "height": 54.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 7258065,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          -54.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "mmBRp72ZBjsEwIKe",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "crP78EQCqvjbbS4V",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "pgkbYedmZ66ASgcm",
      "type": "rectangle",
      "x": 1560.0,
      "y": 448.0,
      "width": 360.0,
      "height": 79.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2380321,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "PWFDf8MN"
        },
        {
          "type": "arrow",
          "id": "0hOSezZ8"
        },
        {
          "type": "arrow",
          "id": "H3PoDiYb"
        }
      ],
      "updated": 1789190079209,
      "link": null,
      "locked": false
    },
    {
      "id": "PWFDf8MN",
      "type": "text",
      "x": 1600.5,
      "y": 453.0,
      "width": 279.0,
      "height": 69.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 5365454,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "text": "Propagates adjoint error vector\ndelta upstream via transposed\nweights and local derivative.",
      "rawText": "Propagates adjoint error vector delta upstream via transposed weights and local derivative.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "pgkbYedmZ66ASgcm",
      "originalText": "Propagates adjoint error vector\ndelta upstream via transposed\nweights and local derivative.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "0hOSezZ8",
      "type": "arrow",
      "x": 1440.0,
      "y": 487.5,
      "width": 120.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 8373997,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "crP78EQCqvjbbS4V",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "pgkbYedmZ66ASgcm",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "oLcfF0PLazxfwokN",
      "type": "rectangle",
      "x": 1200.0,
      "y": 557.0,
      "width": 240.0,
      "height": 79.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7527120,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "2dBPgAOJ"
        },
        {
          "type": "arrow",
          "id": "PS6eyebr"
        },
        {
          "type": "arrow",
          "id": "iGVW1oi1"
        }
      ],
      "updated": 1789190079209,
      "link": null,
      "locked": false
    },
    {
      "id": "2dBPgAOJ",
      "type": "text",
      "x": 1239.0,
      "y": 562.0,
      "width": 162.0,
      "height": 69.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 1793502,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "text": "grad_W =\ntorch.outer(delta,\na_prev)",
      "rawText": "grad_W = torch.outer(delta, a_prev)",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "oLcfF0PLazxfwokN",
      "originalText": "grad_W =\ntorch.outer(delta,\na_prev)",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "PS6eyebr",
      "type": "arrow",
      "x": 1080.0,
      "y": 542.0,
      "width": 120.0,
      "height": 54.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 295538,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          54.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "mmBRp72ZBjsEwIKe",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "oLcfF0PLazxfwokN",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "BsAAiinI4ovvwXFh",
      "type": "rectangle",
      "x": 1560.0,
      "y": 557.0,
      "width": 360.0,
      "height": 79.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6153712,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "PCn8ab18"
        },
        {
          "type": "arrow",
          "id": "iGVW1oi1"
        },
        {
          "type": "arrow",
          "id": "rINLOvaE"
        }
      ],
      "updated": 1789190079209,
      "link": null,
      "locked": false
    },
    {
      "id": "PCn8ab18",
      "type": "text",
      "x": 1596.0,
      "y": 562.0,
      "width": 288.0,
      "height": 69.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2788560,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "text": "Calculates exact weight gradient\nas outer product of error and\nactivations.",
      "rawText": "Calculates exact weight gradient as outer product of error and activations.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "BsAAiinI4ovvwXFh",
      "originalText": "Calculates exact weight gradient\nas outer product of error and\nactivations.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "iGVW1oi1",
      "type": "arrow",
      "x": 1440.0,
      "y": 596.5,
      "width": 120.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 4012198,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "oLcfF0PLazxfwokN",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "BsAAiinI4ovvwXFh",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "jOyLMsaMJfGkXnFS",
      "type": "rectangle",
      "x": 2060.0,
      "y": 468.0,
      "width": 380.0,
      "height": 148.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 5503463,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "Zfro7qRb"
        },
        {
          "type": "arrow",
          "id": "H3PoDiYb"
        },
        {
          "type": "arrow",
          "id": "rINLOvaE"
        }
      ],
      "updated": 1789190079209,
      "link": null,
      "locked": false
    },
    {
      "id": "Zfro7qRb",
      "type": "text",
      "x": 2097.0,
      "y": 473.0,
      "width": 306.0,
      "height": 138.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2543041,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "text": "Reverse-Mode Auto-Differentiation:\nChain Rule: dL/dW = delta *\n(a_prev)^T\nAdjoint error vectors propagate\nperturbation backward in reverse\ntopological order in O(1) pass.",
      "rawText": "Reverse-Mode Auto-Differentiation: Chain Rule: dL/dW = delta * (a_prev)^T Adjoint error vectors propagate perturbation backward in reverse topological order in O(1) pass.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "jOyLMsaMJfGkXnFS",
      "originalText": "Reverse-Mode Auto-Differentiation:\nChain Rule: dL/dW = delta *\n(a_prev)^T\nAdjoint error vectors propagate\nperturbation backward in reverse\ntopological order in O(1) pass.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "H3PoDiYb",
      "type": "arrow",
      "x": 1920.0,
      "y": 487.5,
      "width": 140.0,
      "height": 54.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 4014004,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079209,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          140.0,
          54.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "pgkbYedmZ66ASgcm",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "jOyLMsaMJfGkXnFS",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "rINLOvaE",
      "type": "arrow",
      "x": 1920.0,
      "y": 596.5,
      "width": 140.0,
      "height": 54.5,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 7375750,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079213,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          140.0,
          -54.5
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "BsAAiinI4ovvwXFh",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "jOyLMsaMJfGkXnFS",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "qsjj1PtxvvUkkMvi",
      "type": "rectangle",
      "x": 880.0,
      "y": 756.75,
      "width": 200.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2392346,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "NK8YDWe4"
        },
        {
          "type": "arrow",
          "id": "uhB2hBsH"
        },
        {
          "type": "arrow",
          "id": "QURUPjnN"
        },
        {
          "type": "arrow",
          "id": "kj5Ifm6v"
        }
      ],
      "updated": 1789190079213,
      "link": null,
      "locked": false
    },
    {
      "id": "NK8YDWe4",
      "type": "text",
      "x": 908.0,
      "y": 765.8,
      "width": 144.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6496189,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079213,
      "link": null,
      "locked": false,
      "text": "Optimizer Update\nStep",
      "rawText": "Optimizer Update Step",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "qsjj1PtxvvUkkMvi",
      "originalText": "Optimizer Update\nStep",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "uhB2hBsH",
      "type": "arrow",
      "x": 741.0,
      "y": 543.875,
      "width": 139.0,
      "height": 244.875,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 5616499,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079215,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          139.0,
          244.875
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "gbOfEVJIG5WdjTLU",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "qsjj1PtxvvUkkMvi",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "RgwOFiyLgv6v33qf",
      "type": "rectangle",
      "x": 1200.0,
      "y": 706.0,
      "width": 240.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 3191749,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "t3MEEid7"
        },
        {
          "type": "arrow",
          "id": "QURUPjnN"
        },
        {
          "type": "arrow",
          "id": "WCB3kGSp"
        }
      ],
      "updated": 1789190079215,
      "link": null,
      "locked": false
    },
    {
      "id": "t3MEEid7",
      "type": "text",
      "x": 1243.5,
      "y": 715.0,
      "width": 153.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6403927,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079215,
      "link": null,
      "locked": false,
      "text": "W.data.add_(-lr *\ngrad_W)",
      "rawText": "W.data.add_(-lr * grad_W)",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "RgwOFiyLgv6v33qf",
      "originalText": "W.data.add_(-lr *\ngrad_W)",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "QURUPjnN",
      "type": "arrow",
      "x": 1080.0,
      "y": 788.75,
      "width": 120.0,
      "height": 50.75,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 8848331,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079215,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          -50.75
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "qsjj1PtxvvUkkMvi",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "RgwOFiyLgv6v33qf",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "5aZBfz9NcEyDzCG1",
      "type": "rectangle",
      "x": 1560.0,
      "y": 706.0,
      "width": 360.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7406225,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "XqWbeJ0p"
        },
        {
          "type": "arrow",
          "id": "WCB3kGSp"
        },
        {
          "type": "arrow",
          "id": "yRC6yBxW"
        }
      ],
      "updated": 1789190079215,
      "link": null,
      "locked": false
    },
    {
      "id": "XqWbeJ0p",
      "type": "text",
      "x": 1587.0,
      "y": 715.0,
      "width": 306.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 3288684,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079215,
      "link": null,
      "locked": false,
      "text": "Adjusts weights against the\ndirection of steepest loss ascent.",
      "rawText": "Adjusts weights against the direction of steepest loss ascent.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "5aZBfz9NcEyDzCG1",
      "originalText": "Adjusts weights against the\ndirection of steepest loss ascent.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "WCB3kGSp",
      "type": "arrow",
      "x": 1440.0,
      "y": 738.0,
      "width": 120.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 3870871,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079215,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "RgwOFiyLgv6v33qf",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "5aZBfz9NcEyDzCG1",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "GmPCUu9UoTi3SJ4j",
      "type": "rectangle",
      "x": 1200.0,
      "y": 807.5,
      "width": 240.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7554627,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "PdGVtqy7"
        },
        {
          "type": "arrow",
          "id": "kj5Ifm6v"
        },
        {
          "type": "arrow",
          "id": "GnzDxJ6N"
        }
      ],
      "updated": 1789190079215,
      "link": null,
      "locked": false
    },
    {
      "id": "PdGVtqy7",
      "type": "text",
      "x": 1230.0,
      "y": 816.5,
      "width": 180.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 9889083,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079215,
      "link": null,
      "locked": false,
      "text": "optimizer.zero_grad(\nset_to_none=True)",
      "rawText": "optimizer.zero_grad( set_to_none=True)",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "GmPCUu9UoTi3SJ4j",
      "originalText": "optimizer.zero_grad(\nset_to_none=True)",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "kj5Ifm6v",
      "type": "arrow",
      "x": 1080.0,
      "y": 788.75,
      "width": 120.0,
      "height": 50.75,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 9553054,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          50.75
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "qsjj1PtxvvUkkMvi",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "GmPCUu9UoTi3SJ4j",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "VnAJ6pS2ov3aIzmW",
      "type": "rectangle",
      "x": 1560.0,
      "y": 800.0,
      "width": 360.0,
      "height": 79.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7501795,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "7IRToi2t"
        },
        {
          "type": "arrow",
          "id": "GnzDxJ6N"
        },
        {
          "type": "arrow",
          "id": "cLqz0miw"
        }
      ],
      "updated": 1789190079216,
      "link": null,
      "locked": false
    },
    {
      "id": "7IRToi2t",
      "type": "text",
      "x": 1587.0,
      "y": 805.0,
      "width": 306.0,
      "height": 69.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 9724516,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "text": "Frees accumulated gradient buffers\nto prevent gradient leakage across\ntraining steps.",
      "rawText": "Frees accumulated gradient buffers to prevent gradient leakage across training steps.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "VnAJ6pS2ov3aIzmW",
      "originalText": "Frees accumulated gradient buffers\nto prevent gradient leakage across\ntraining steps.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "GnzDxJ6N",
      "type": "arrow",
      "x": 1440.0,
      "y": 839.5,
      "width": 120.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 2184815,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          120.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "GmPCUu9UoTi3SJ4j",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "VnAJ6pS2ov3aIzmW",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "BP669AguJJ0yDtjs",
      "type": "rectangle",
      "x": 2060.0,
      "y": 714.75,
      "width": 380.0,
      "height": 148.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6647943,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "tQRVM68K"
        },
        {
          "type": "arrow",
          "id": "yRC6yBxW"
        },
        {
          "type": "arrow",
          "id": "cLqz0miw"
        }
      ],
      "updated": 1789190079216,
      "link": null,
      "locked": false
    },
    {
      "id": "tQRVM68K",
      "type": "text",
      "x": 2088.0,
      "y": 719.8,
      "width": 324.0,
      "height": 138.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7673636,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "text": "Numerical Stability in Deep Nets:\nGradient clipping bounds parameter\nupdates.\nAMP dynamic loss scaling avoids FP16\nunderflow to zero on subtle weight\nadjustments.",
      "rawText": "Numerical Stability in Deep Nets: Gradient clipping bounds parameter updates. AMP dynamic loss scaling avoids FP16 underflow to zero on subtle weight adjustments.",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "BP669AguJJ0yDtjs",
      "originalText": "Numerical Stability in Deep Nets:\nGradient clipping bounds parameter\nupdates.\nAMP dynamic loss scaling avoids FP16\nunderflow to zero on subtle weight\nadjustments.",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "yRC6yBxW",
      "type": "arrow",
      "x": 1920.0,
      "y": 738.0,
      "width": 140.0,
      "height": 50.75,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 9833486,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          140.0,
          50.75
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "5aZBfz9NcEyDzCG1",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "BP669AguJJ0yDtjs",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "cLqz0miw",
      "type": "arrow",
      "x": 1920.0,
      "y": 839.5,
      "width": 140.0,
      "height": 50.75,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 4781985,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          140.0,
          -50.75
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "VnAJ6pS2ov3aIzmW",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "BP669AguJJ0yDtjs",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "SHXa63kw1Lb0LSZk",
      "type": "rectangle",
      "x": 880.0,
      "y": 1150.0,
      "width": 190.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 986806,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "NpwTGR5R"
        },
        {
          "type": "arrow",
          "id": "md8eltjB"
        },
        {
          "type": "arrow",
          "id": "Qz0fn8iz"
        }
      ],
      "updated": 1789190079216,
      "link": null,
      "locked": false
    },
    {
      "id": "NpwTGR5R",
      "type": "text",
      "x": 903.0,
      "y": 1170.5,
      "width": 144.0,
      "height": 23.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6274650,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "text": "Input Tensor a_0",
      "rawText": "Input Tensor a_0",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "SHXa63kw1Lb0LSZk",
      "originalText": "Input Tensor a_0",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "md8eltjB",
      "type": "arrow",
      "x": 741.0,
      "y": 1182.0,
      "width": 139.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 9512721,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079216,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          139.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "ctRogZEC6MxttOQf",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "SHXa63kw1Lb0LSZk",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "RMEwRtd8hwngZ1Ab",
      "type": "rectangle",
      "x": 1135.0,
      "y": 1150.0,
      "width": 190.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2617050,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "LyMIXPw4"
        },
        {
          "type": "arrow",
          "id": "Qz0fn8iz"
        },
        {
          "type": "arrow",
          "id": "7Nmy52pq"
        }
      ],
      "updated": 1789190079217,
      "link": null,
      "locked": false
    },
    {
      "id": "LyMIXPw4",
      "type": "text",
      "x": 1171.5,
      "y": 1159.0,
      "width": 117.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7234428,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079217,
      "link": null,
      "locked": false,
      "text": "Affine Linear\nSum Wx+b",
      "rawText": "Affine Linear Sum Wx+b",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "RMEwRtd8hwngZ1Ab",
      "originalText": "Affine Linear\nSum Wx+b",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "Qz0fn8iz",
      "type": "arrow",
      "x": 1070.0,
      "y": 1182.0,
      "width": 65.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 4873131,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079217,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          65.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "SHXa63kw1Lb0LSZk",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "RMEwRtd8hwngZ1Ab",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "Fd8LvCLe84eOVLeA",
      "type": "rectangle",
      "x": 1390.0,
      "y": 1150.0,
      "width": 190.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 2998129,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "0gVbNCxy"
        },
        {
          "type": "arrow",
          "id": "7Nmy52pq"
        },
        {
          "type": "arrow",
          "id": "DAmCCRPh"
        }
      ],
      "updated": 1789190079217,
      "link": null,
      "locked": false
    },
    {
      "id": "0gVbNCxy",
      "type": "text",
      "x": 1417.5,
      "y": 1159.0,
      "width": 135.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 7293084,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079217,
      "link": null,
      "locked": false,
      "text": "Activation Gate\nsigma(z)",
      "rawText": "Activation Gate sigma(z)",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "Fd8LvCLe84eOVLeA",
      "originalText": "Activation Gate\nsigma(z)",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "7Nmy52pq",
      "type": "arrow",
      "x": 1325.0,
      "y": 1182.0,
      "width": 65.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 1323652,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079217,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          65.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "RMEwRtd8hwngZ1Ab",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "Fd8LvCLe84eOVLeA",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "9AIA6TOBOurLbKLA",
      "type": "rectangle",
      "x": 1645.0,
      "y": 1150.0,
      "width": 190.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6133782,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "wBMkhNWy"
        },
        {
          "type": "arrow",
          "id": "DAmCCRPh"
        },
        {
          "type": "arrow",
          "id": "IAXwcfBG"
        },
        {
          "type": "arrow",
          "id": "FWm3yOJD"
        }
      ],
      "updated": 1789190079218,
      "link": null,
      "locked": false
    },
    {
      "id": "wBMkhNWy",
      "type": "text",
      "x": 1677.0,
      "y": 1159.0,
      "width": 126.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 4991174,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079218,
      "link": null,
      "locked": false,
      "text": "Loss Criterion\nL(y, y_hat)",
      "rawText": "Loss Criterion L(y, y_hat)",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "9AIA6TOBOurLbKLA",
      "originalText": "Loss Criterion\nL(y, y_hat)",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "DAmCCRPh",
      "type": "arrow",
      "x": 1580.0,
      "y": 1182.0,
      "width": 65.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 3247974,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079218,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          65.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "Fd8LvCLe84eOVLeA",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "9AIA6TOBOurLbKLA",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "0D1QIHO1h38sntzR",
      "type": "rectangle",
      "x": 1900.0,
      "y": 1150.0,
      "width": 190.0,
      "height": 64.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 6610036,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "Wl7YjRwj"
        },
        {
          "type": "arrow",
          "id": "IAXwcfBG"
        }
      ],
      "updated": 1789190079218,
      "link": null,
      "locked": false
    },
    {
      "id": "Wl7YjRwj",
      "type": "text",
      "x": 1950.0,
      "y": 1159.0,
      "width": 90.0,
      "height": 46.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 1257576,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079218,
      "link": null,
      "locked": false,
      "text": "Accumulate\nGradients",
      "rawText": "Accumulate Gradients",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "0D1QIHO1h38sntzR",
      "originalText": "Accumulate\nGradients",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "IAXwcfBG",
      "type": "arrow",
      "x": 1835.0,
      "y": 1182.0,
      "width": 65.0,
      "height": 0.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 2704782,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079218,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          65.0,
          0.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "9AIA6TOBOurLbKLA",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "0D1QIHO1h38sntzR",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    },
    {
      "id": "iE8lLXGzcGYs6Ry0",
      "type": "rectangle",
      "x": 1645.0,
      "y": 1030.0,
      "width": 190.0,
      "height": 79.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 4856644,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": [
        {
          "type": "text",
          "id": "3yNoZKWG"
        },
        {
          "type": "arrow",
          "id": "FWm3yOJD"
        }
      ],
      "updated": 1789190079218,
      "link": null,
      "locked": false
    },
    {
      "id": "3yNoZKWG",
      "type": "text",
      "x": 1686.0,
      "y": 1035.0,
      "width": 108.0,
      "height": 69.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 2,
      "strokeStyle": "solid",
      "roughness": 1,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": null,
      "seed": 3099097,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079218,
      "link": null,
      "locked": false,
      "text": "Compute Loss\nAdjoint\ndL/dy_hat",
      "rawText": "Compute Loss Adjoint dL/dy_hat",
      "fontSize": 20,
      "fontFamily": 7,
      "textAlign": "center",
      "verticalAlign": "middle",
      "containerId": "iE8lLXGzcGYs6Ry0",
      "originalText": "Compute Loss\nAdjoint\ndL/dy_hat",
      "hasTextLink": false,
      "autoResize": true,
      "lineHeight": 1.15
    },
    {
      "id": "FWm3yOJD",
      "type": "arrow",
      "x": 1740.0,
      "y": 1150.0,
      "width": 0.0,
      "height": 41.0,
      "angle": 0,
      "strokeColor": "#1e1e1e",
      "backgroundColor": "transparent",
      "fillStyle": "solid",
      "strokeWidth": 4,
      "strokeStyle": "solid",
      "roughness": 0,
      "opacity": 100,
      "groupIds": [],
      "frameId": null,
      "roundness": {
        "type": 2
      },
      "seed": 7582010,
      "version": 1,
      "versionNonce": 1,
      "isDeleted": false,
      "boundElements": null,
      "updated": 1789190079218,
      "link": null,
      "locked": false,
      "points": [
        [
          0.0,
          0.0
        ],
        [
          0.0,
          -41.0
        ]
      ],
      "lastCommittedPoint": null,
      "startBinding": {
        "elementId": "9AIA6TOBOurLbKLA",
        "focus": 0,
        "gap": 1
      },
      "endBinding": {
        "elementId": "iE8lLXGzcGYs6Ry0",
        "focus": 0,
        "gap": 1
      },
      "startArrowhead": null,
      "endArrowhead": "arrow"
    }
  ],
  "appState": {
    "theme": "dark",
    "viewBackgroundColor": "#ffffff",
    "currentItemStrokeColor": "#1e1e1e",
    "currentItemBackgroundColor": "transparent",
    "currentItemFillStyle": "solid",
    "currentItemStrokeWidthKey": "bold",
    "currentItemStrokeStyle": "solid",
    "currentItemRoughness": 0,
    "currentItemOpacity": 100,
    "currentItemFontFamily": 7,
    "currentItemFontSize": 20,
    "currentItemTextAlign": "center",
    "currentItemEndArrowhead": "arrow",
    "gridSize": 20,
    "zoom": {
      "value": 0.4
    }
  },
  "files": {}
}
```
%%
